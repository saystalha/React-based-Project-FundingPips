    ...
    if (window.confirm("Are you absolutely sure? This will permanently delete your account and all associated data.")) {
      try {
        ...
        if (error.code === 'auth/requires-recent-login') {
          alert("Security Sensitive: Please logout and login again before deleting your account.");
        }
      }
    }
  };
  ...
  return (
    <nav className="bg-[#0a0a0a] border-b border-white/5 px-8 sticky top-0 z-50 backdrop-blur-md bg-opacity-80">
      <div className="max-w-7xl mx-auto flex justify-between items-center min-h-[80px]">
        ...
        {/* Left: Brand */}
        <Link to="/" className="text-2xl font-bold text-electric flex items-center gap-2 hover:opacity-80 transition-opacity">
        ...
        <div className="flex items-center gap-4">
          {!user ? (
            <div className="hidden md:flex items-center gap-3">
              ...
              <Link to="/signup" className="px-5 py-2 bg-electric hover:bg-blue-600 text-white text-sm font-bold rounded-lg transition-all shadow-lg shadow-blue-500/20">
                Get Started
              </Link>
            </div>
          ) : (
            <div className="relative" onMouseLeave={() => setShowDropdown(false)}>
              ...
              >
                ...
                <div className="hidden sm:block text-left">
                  <p className="text-[10px] text-gray-500 font-bold uppercase leading-none mb-1">Trader</p>
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-3 w-64 bg-[#0f172a] border border-white/10 rounded-xl shadow-2xl py-3 z-[60] backdrop-blur-xl animate-in fade-in zoom-in duration-200">
                  <div className="px-4 py-3 border-b border-white/5">
                    <p className="text-[10px] uppercase tracking-widest text-gray-500 font-bold">Account Session</p>
                    <p className="text-sm font-medium text-white truncate mt-1">{user.email}</p>
                  </div>
                  
                  <div className="py-2">
                    <Link to="/dashboard" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-300 hover:bg-electric/10 hover:text-electric transition-all">
                       <LayoutDashboard size={16} /> Dashboard
                    </Link>
                    <Link to="/settings" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-300 hover:bg-electric/10 hover:text-electric transition-all">
                       <Settings size={16} /> Settings
                    </Link>
                  </div>

                  <div className="mt-2 pt-2 border-t border-white/5">
                    <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-2 text-sm text-gray-400 hover:bg-white/5 hover:text-white transition-all">
                      <LogOut size={16} /> Logout
                    </button>
                    <button onClick={handleDeleteAccount} className="flex items-center gap-3 w-full px-4 py-2 text-sm text-red-500/80 hover:bg-red-500/10 hover:text-red-500 font-medium transition-all">
