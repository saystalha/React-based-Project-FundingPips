...
const FirmDetails = () => {
    ...
    const fetchFirmDetail = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Determine management permissions: admin or owner
  ...
  const handleUpdate = async () => {
    if (!canManage) {
      alert("You do not have permission to update this entry.");
      return;
    }
    try {
      const docRef = doc(db, "Firm", id);
  ...
  if (!firmEntry) {
    return (
      <div className="p-8 text-center bg-card rounded-xl border border-white/10">
        ...
        <Link to="/firmtracker" className="text-electric hover:underline">
          ← Back to Tracker
        </Link>
      </div>
    );
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        {canManage ? (
          ...
          <div className="text-sm text-muted">
            You can view this submission. Only the owner or an admin can edit or
            delete.
          </div>
        )}
      </div>
      ...
      <div className="bg-card rounded-2xl border border-white/5 overflow-hidden shadow-2xl">
        <div
          className={`h-2 w-full ${
            firmEntry.inquiryType === "Technical Issue"
              ? "bg-red-500"
              : "bg-electric"
          }`}
        ></div>
            ...
            <div className="space-y-1">
              ...
              {isEditing ? (
                <input
                  ...
                  onChange={(e) =>
                    setEditData({ ...editData, traderId: e.target.value })
                  }
                />
              ) : (
                <p className="text-white text-xl font-bold">
                  {firmEntry.traderId || "N/A"}
                </p>
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              {isEditing ? (
                ...
                <p className="text-electric font-mono">
                  {firmEntry.email || "No email"}
                </p>
              )}
            </div>

            <div>
              <label className="text-xs uppercase tracking-widest text-muted font-bold block mb-1">
                Inquiry Category
              </label>
              <span
